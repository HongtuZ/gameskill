...
检查规则:
    ...
    2. 当 need_guide 为 "是" 时，必须包含 guide_text, situation, guidance_reason, ego_name
    3. 同一叶子目录下连续编号的json文件中，若 map_name 或 ego_name 发生变化，给出 warning 提示
"""
...
def check_single_file(filepath, data):
    ...
    # 1. description 是必填项
    game_view = safe_str(data.get("game_view", "非局内"))
    description = safe_str(reasons.get("description", ""))
    if game_view != "非局内" and not description.strip():
        errors.append(f"视角:{game_view} description 为空（必填项）")

    # 2. need_guide 为 "是" 时的必填字段检查
    guide = data.get("guide", {})
    need_guide = guide.get("need")
    if need_guide:
        # guide_text
        guide_text = safe_str(guide.get("advice", ""))
        ...
        # ego_name
        ego_name = safe_str(data.get("ego_name", ""))
        if not ego_name.strip():
            errors.append("need_guide=是 时 ego_name 不能为空")
    ...
    # ego_name 为空（非 need_guide 场景）
    ego_name = safe_str(data.get("ego_name", ""))
    if (not ego_name.strip()) and need_guide != "是":
        warnings.append("ego_name 为空")
...
def check_transitions(sorted_files):
    """
    检查连续编号文件中 map_name 和 ego_name 的变化。
    ...
    for filepath in sorted_files:
        ...
        if prev_data is not None:
            changes = []
            for field in ["map_name", "ego_name"]:
